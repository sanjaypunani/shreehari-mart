...
export class MonthlyBillRepository {
  ...
  async bulkGenerateForMonth(
    ...
    for (const customer of customers) {
      try {
        // Find orders for this customer in this month that are not yet billed
        const startDate = new Date(year, parseInt(month) - 1, 1);
        const endDate = new Date(year, parseInt(month), 0);

        const unbilledOrders = await this.orderRepository.find({
          where: {
        ...
        if (unbilledOrders.length === 0) {
          continue; // No new orders to bill
        }
        ...
        if (existingBill) {
          // First, link the new orders to the existing bill
          await this.orderRepository.update(
            unbilledOrders.map((o) => o.id),
            { monthlyBillId: existingBill.id }
          );

          // Now get all orders associated with this bill to recalculate total
          const allBillOrders = await this.orderRepository.find({
            where: {
              monthlyBillId: existingBill.id,
              paymentMode: 'monthly',
            },
          ...
          // Recalculate total amount and order count from all associated orders
          const totalAmount = allBillOrders.reduce(
            (sum, order) => sum + Number(order.totalAmount),
            0
          );

          // Update the bill with recalculated values
          await this.update(existingBill.id, {
            totalAmount,
            orderCount: allBillOrders.length,
          });
        ...
        result.errors.push(
          `Error processing bill for customer ${customer.name}: ${errorMessage}`
        );
      }
    }
